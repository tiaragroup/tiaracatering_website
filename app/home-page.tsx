...
const copy = {
  ...
  ar: {
    ...
    brands: [["فلور سنت", "زهور", "حيث يلتقي العطر بالأناقة — تنسيقات زهور تحكي قصصاً.", "Asset-2-370x343.png"], ["بيكرز بيكري", "مخبوزات", "مخبوزات طازجة من وصفات تقليدية وأجود المكونات.", "Asset-3-370x343.png"], ["إليمنتس دو شوكولا", "شوكولاتة", "فن الشوكولاتة الفاخرة من أجود الكاكاو ومزيج راقٍ من النكهات.", "Asset-1-370x343.png"]],
...
export default function HomePage({ lang = "en" }: { lang?: Lang }) {
  ...
  return <main dir={t.dir} className={`site-shell ${lang}`}>
    ...
    <footer className="site-footer"><div className="footer-brand"><img src="/tiara-logo.png" alt="Tiara Catering" /><p>{t.footerBody}</p></div><div><span>{lang === "ar" ? "تواصل" : "Connect"}</span><a href={CALL_HREF}>{lang === "ar" ? "اتصل" : "Call"}: {CALL_DISPLAY}</a>{socialLinks.map(({ label, href }) => <a href={href} key={label} target="_blank" rel="noopener noreferrer" aria-label={`${label} — Tiara Catering`}>{label}</a>)}<a href={WHATSAPP_URL}>WhatsApp</a></div><div><span>{lang === "ar" ? "اكتشف" : "Explore"}</span>{t.nav.slice(0, 4).map(([label, href]) => <a href={href} key={href}>{label}</a>)}</div><small>{t.rights}</small></footer>
