...
const copy = {
  en: {
    ...
    services: [
      ...
      ["Event Consulting", "Concept, planning, vendor coordination and on-site management in one place.", "/images/event-consulting-corporate-catering.png"],
    ],
    ...
    brands: [["Flower Scent", "Florist", "Where fragrance meets elegance — floral arrangements that tell stories.", "flower_scent_exact_white_background.png", "https://flowerscent.sa/"], ["Baker’s Bakery", "Bakery", "Freshly baked goodness made from traditional recipes and fine ingredients.", "bakers-bakery.png", "http://bakerisbakery.com/"], ["éléments du chocolat", "Chocolatier", "The art of fine chocolate, crafted from exceptional cocoa and delicate flavours.", "elements-du-chocolat-chocolate-brand-shop-in-riyadh.png", "https://elementsduchocolat.com/"]],
  ...
  ar: {
    ...
    viewServices: "استعرض الخدمات", proof: [["1200+", "مناسبة نُفّذت"], ["5", "خطوط خدمة"], ["3", "بيوت متخصصة"]],
    ...
    brands: [["فلور سنت", "زهور", "حيث يلتقي العطر بالأناقة — تنسيقات زهور تحكي قصصاً.", "flower_scent_exact_white_background.png", "https://flowerscent.sa/"], ["بيكرز بيكري", "مخبوزات", "مخبوزات طازجة من وصفات تقليدية وأجود المكونات.", "bakers-bakery.png", "http://bakerisbakery.com/"], ["إليمنتس دو شوكولا", "شوكولاتة", "فن الشوكولاتة الفاخرة من أجود الكاكاو ومزيج راقٍ من النكهات.", "elements-du-chocolat-chocolate-brand-shop-in-riyadh.png", "https://elementsduchocolat.com/"]],
...
const handleTouchEnd = () => {
  ...
  function scrollToQuotationFormOnMobile(event: React.MouseEvent<HTMLAnchorElement>) {
  ...
  return <main dir={t.dir} className={`site-shell ${lang}`}>
      <section id="top" className="hero">
        <div className="hero-copy"><p className="eyebrow reveal"><span />{t.eyebrow}</p><h1 className={lang === "ar" ? "arabic-hero-title" : undefined}>{lang === "ar" ? <><span>كن ضيفاً في</span><span>مناسبتك <em>الخاصة.</em></span></> : t.hero.map((line, i) => i === 2 ? <em key={line}>{line}</em> : <span key={line}>{line}</span>)}</h1><p className="lead">{t.intro}</p><div className="buttons">
  <a className="pill dark font-ar-15" href="#contact" onClick={scrollToQuotationFormOnMobile}>
    {t.quote}
    <ArrowUpRight size={16} strokeWidth={1.5} aria-hidden="true" />
  </a>

  <a className="text-link font-ar-15" href="#services">
    {t.viewServices}
  ...
  <div className="gallery-rail">
    {gallery.map((src, i) => (
      ...
      <section id="brands" className="section brands"><div className="section-head"><div><p className="kicker">
        {t.brandsTag}</p>
        <h2>{t.brandsTitle}</h2>
        </div>
        </div>
        <div className="brand-grid">{t.brands.map(([title, tag, desc, image, url], i) => { const content = <><div className="brand-image"><span>0{i + 1}</span><img src={`/images/${image}`} alt={title} loading="lazy" /></div><small>{tag}</small><div className="brand-card-heading"><h3>{title}</h3>{url && <span aria-hidden="true"><ArrowUpRight size={16} strokeWidth={1.5} aria-hidden="true" /></span>}</div><p className="font-ar-15">{desc}</p></>; return url ? <a className="brand-card linked" href={url} target="_blank" rel="noopener noreferrer" aria-label={`${title} — ${lang === "ar" ? "زيارة الموقع" : "Visit website"}`} key={title}>{content}</a> : <article className="brand-card" key={title}>{content}</article>; })}</div></section>
